import { profile, researchPaths, prepareResearchFiles } from './scripts/frontend-profile.mjs';
if (profile === 'research') prepareResearchFiles();
...
const config = {
	...
	kit: {
		...(profile === 'research' ? { files: researchPaths } : {}),
		typescript: {
			config: (config) => {
				// Editors/checkers should inspect canonical routes once, not both copies.
				if (profile === 'research') {
					config.include = config.include.filter((path) => !path.includes('.generated/research/routes'));
				}
			}
		},
