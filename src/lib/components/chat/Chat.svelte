<script lang="ts">
	...
	onMount(() => {
		...
		const showEssaySidebarSubscribe = showEssaySidebar.subscribe(async (value) => {
			await tick();
			if (value && !$mobile) {
				try {
					essayPaneComponent?.openPane();
				} catch (e) {
					// ignore
				}
			}
...
>
	{#if !loading}
		<div in:fade={{ duration: 50 }} class="relative flex h-full w-full min-w-0">
			...
			<div class="h-full min-w-0 flex-1">
				<PaneGroup direction="horizontal" class="h-full w-full">
					<Pane
						...
						defaultSize={50}
						minSize={20}
						class="h-full min-w-0 flex relative flex-col"
					>
