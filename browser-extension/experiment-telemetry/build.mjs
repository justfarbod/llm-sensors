...
if (output === root || output === resolve('/')) throw new Error('Refusing to overwrite an unsafe output path.');
const rawOrigin = process.env.EXPERIMENT_TELEMETRY_EXTENSION_ORIGIN ?? '';
...
for (const name of ['background.js', 'content.js', 'tab-normalization.js', 'README.md'])
